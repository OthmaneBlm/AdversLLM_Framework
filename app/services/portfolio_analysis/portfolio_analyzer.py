...
class PortfolioAnalyzer:
    ...
    def _generate_geographical_allocations_string(self, product_allocation, portfolios):
        current_portfolio = self.portfolio_weights.get("Current", {})
        simulated_portfolio = self.portfolio_weights.get("Simulated", {})
        
        relevant_portfolios = set(
            list(current_portfolio.keys())
            + list(simulated_portfolio.keys())
        )
        ...
        for _, row in product_allocation.iterrows():
            ...
            elif row["product_code"] == "SIM Boost":
                full_string += format_allocations(
                    calculate_portfolio_geo_allocation(simulated_portfolio),
                    "Simulated portfolio geographical allocation:"
                )
        comparison_red_str = "-> Simulated portfolio reduces exposure to:\n"
        comparison_inc_str = "Simulated portfolio increases exposure to:\n"

        countries_total_current = calculate_portfolio_geo_allocation(current_portfolio)
        countries_total_simulated = calculate_portfolio_geo_allocation(simulated_portfolio)

        for country, current_value in countries_total_current.items():
            simulated_value = countries_total_simulated.get(country, 0)
            diff = simulated_value - current_value
            if diff < 0:
                comparison_red_str += f"{country} ({diff:.0%}), "
            elif diff > 0:
                comparison_inc_str += f"{country} (+{diff:.0%}), "

        for country, simulated_value in countries_total_simulated.items():
            if country not in countries_total_current:
                comparison_inc_str += f"{country} (+{simulated_value:.0%}), "

        full_string += comparison_red_str.rstrip(", ") + "\n" + comparison_inc_str.rstrip(", ")
        
        # TODO: text nog toevoegen
        return full_string

    def _generate_sector_allocations_string(self, product_allocation, portfolios):
        current_portfolio = self.portfolio_weights.get("Current", {})
        simulated_portfolio = self.portfolio_weights.get("Simulated", {})
        relevant_portfolios = set(
            list(current_portfolio.keys())
        ...
        for _, row in product_allocation.iterrows():
            if row["product_name "] in relevant_portfolios:
                ...
                full_string += format_allocations(
                    calculate_portfolio_allocation(simulated_portfolio),
                    "Simulated portfolio sector allocation:"
                )

        # TODO: text nog toevoegen
        return full_string

    def _generate_asset_allocations_string(self, product_allocation, portfolios):
        current_portfolio = self.portfolio_weights.get("Current", {})
        simulated_portfolio = self.portfolio_weights.get("Simulated", {})
        relevant_portfolios = set(current_portfolio.keys()).union(simulated_portfolio.keys())
        ...
        for _, row in product_allocation.iterrows():
            if row["product_name "] in relevant_portfolios:
                ...
                full_string += format_allocations(
                    calculate_befo_allocation(simulated_portfolio),
    ...
    def _generate_PAI_string(self, all_pais, product_ESG_data):
        ...
        for pai in all_pais:
            full_string += f"-  {pai}\n"
            current_value = get_values(product_ESG_data, pai, list(current_portfolio.keys()), portfolio=current_portfolio)
            simulated_value = get_values(product_ESG_data, pai, list(simulated_portfolio.keys()), portfolio=simulated_portfolio)
            ...
            full_string += (
                ...
                f"Simulated portfolio : {simulated_value:.2f} – "
    ...
    def _generate_SDG_string(self, all_sdgs, product_ESG_data):
        product_ESG_data[["Date", "asset_code", "asset_name"]] = product_ESG_data[
            ["Date", "asset_code", "asset_name"]
        ].fillna(value="")
        ...
        for sdg in all_sdgs:
            ...
            simulated_value = get_values(product_ESG_data, sdg, list(simulated_portfolio.keys()),  portfolio=simulated_portfolio)
            ...
            full_string += (
                ...
                f"Simulated portfolio : {simulated_value:.2%} – "
    ...
    def _generate_taxonomy_string(self, product_ESG_data, profil):
        full_string = "## Taxonomy \n"
        portfolio_weights = self.portfolio_weights
        # taxonomies["Current"] = product_ESG_data[product_ESG_data["asset_code"].str.contains("Current Boost")]["taxonomy_alignment"].iloc[0]
        # taxonomies["Simulated"] = product_ESG_data[product_ESG_data["asset_code"].str.contains("SIM Boost")]["taxonomy_alignment"].iloc[0]
        # print(taxonomies)
        required_taxonomy = profil["Min_Taxonomie "]
        for port in ["Current", "Simulated"]:
            port_weights = portfolio_weights[port]
            total_taxonomy = 0
            ...
            full_string += f"{port} portfolio: {total_taxonomy:.2%}\n"

            if total_taxonomy < required_taxonomy:
                full_string += f"{port} taxonomy of the portfolio is below the required threshold of {required_taxonomy:.2%}\n"
            elif total_taxonomy > required_taxonomy:
                full_string += f"{port} taxonomy of the portfolio is above the required threshold of {required_taxonomy:.2%}\n"
            else:
                full_string += (
                    f"{port} taxonomy of the portfolio matches the required threshold\n"
                )
        return full_string
    
    def _generate_safir_string(self, product_ESG_data):
        full_string = "## SAFIR:\n"
        av_safir_current, av_esg_safir_current = 0, 0
        av_safir_simulated, av_esg_safir_simulated = 0, 0

    # Helper function to calculate weighted SAFIR values for a given portfolio
        def process_portfolio(portfolio, portfolio_type):
            nonlocal av_safir_current, av_esg_safir_current, av_safir_simulated, av_esg_safir_simulated
            av_safir, av_esg_safir = 0, 0
            full_string = f"### {portfolio_type} Portfolio:\n"
            ...
            # Return the full string and the weighted averages
            full_string += (
                f"{portfolio_type} portfolio asset allocation:\n"
                f"     o SAFIR suitability (weighted average): {av_safir:.2f}\n"
                f"     o SAFIR ESG impact (weighted average): {av_esg_safir:.2f}\n"
            )
            
            return full_string, av_safir, av_esg_safir
        ...
        if current_portfolio:
            current_string, av_safir_current, av_esg_safir_current = process_portfolio(current_portfolio, "Current")
            full_string += current_string
        
        if simulated_portfolio:
            simulated_string, av_safir_simulated, av_esg_safir_simulated = process_portfolio(simulated_portfolio, "Simulated")
            full_string += simulated_string

        return full_string

    def _generate_sfdr_string(self, product_ESG_data, profil, portfolios):
        full_string = "## SFDR \n"
        portfolio_weights = self.portfolio_weights
        ...
        for port in ["Current", "Simulated"]:
            ...
            full_string += f"{port} portfolio: {total_sfdr:.2%}\n"

            if total_sfdr < required_taxonomy:
                full_string += f"{port} SFDR of the portfolio is below the required threshold of {required_taxonomy:.2%}\n"
            elif total_sfdr > required_taxonomy:
                full_string += f"{port} SFDR of the portfolio is above the required threshold of {required_taxonomy:.2%}\n"
            else:
                full_string += (
                    f"{port} SFDR of the portfolio matches the required threshold\n"
                )
        return full_string
    ...
    def generate_data_string(self):
        ...
        portfolio_sections = [
            ...
            self._generate_portfolio_string(profile),
            self._generate_geographical_allocations_string(
                self.product_allocation, self.portfolios
            ),
            self._generate_sector_allocations_string(
                self.product_allocation, self.portfolios
            ),
            self._generate_asset_allocations_string(
                self.product_allocation, self.portfolios
            ),
            self._generate_SDG_string(all_sdgs, self.product_ESG_data),
            self._generate_PAI_string(all_pais, self.product_ESG_data),
            self._generate_taxonomy_string(
                self.product_ESG_data, profile
            ),
            self._generate_safir_string(self.product_ESG_data),
            self._generate_sfdr_string(self.product_ESG_data, profile, self.portfolios),
